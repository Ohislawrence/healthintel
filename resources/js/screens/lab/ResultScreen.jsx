...
export default function ResultScreen() {
    ...
    const handleLanguageChange = async (newLang) => {
        setLang(newLang);
        if (newLang === 'en') { setTranslatedText(null); return; }
        if (!interpretation?.interpretation_text) return;
        try {
            setTranslating(true);
            const res = await api.post(`/submissions/${id}/translate`, { target_language: newLang });
    ...
    const handleDownloadCard = async () => {
        try {
            ...
            const response = await fetch('/api/health-report-card', {
            ...
            if (response.ok) {
                ...
                a.href = url;
                a.download = 'health-report-card.pdf';
    ...
    if (isLoading) {
        return (<div className="max-w-xl mx-auto space-y-4"><div className="skeleton h-8 w-48 rounded" /><div className="skeleton h-64 w-full rounded-xl" /></div>);
    }
    if (isPending) {
        return (
            <div className="max-w-md mx-auto text-center py-16">
                <div className="w-20 h-20 rounded-2xl bg-teal-50 border-2 border-teal-200 flex items-center justify-center mx-auto mb-6"><span className="text-4xl text-teal-500">◐</span></div>
                <p className="text-lg font-bold text-neutral-900 mb-2">{isPdf ? 'Analyzing Your Report' : 'Processing your results...'}</p>
                <p className="text-sm text-neutral-500 mb-6 leading-relaxed">{isPdf ? 'Our AI is reading your lab results and preparing a plain-language explanation...' : 'This usually takes 30-60 seconds'}</p>
                ...
                <div className="card p-5 bg-danger-50 border-danger-200">
                    ...
                    <div className="bg-white rounded-xl p-4">
                        ...
                        {['Use a text-based PDF', 'Ensure report is in English', 'Try uploading a clearer copy', 'Enter values manually instead'].map((tip, i) => (
                            <div key={i} className="flex gap-2 mb-1"><span className="text-sm text-neutral-400 font-bold">·</span><span className="text-sm text-neutral-600">{tip}</span></div>
                        ))}
                    </div>
                </div>
            </div>
        ...
        <div className="max-w-xl mx-auto space-y-5">
            ...
            <div className="card p-5">
                <div className="flex items-center">
                    <div className="flex-1 text-center"><p className="text-[10px] font-bold text-neutral-400 uppercase tracking-wider mb-1">Type</p><p className="text-sm font-semibold text-neutral-900">{isPdf ? 'PDF Upload' : panel.name || 'Lab Result'}</p></div>
                    <div className="w-px h-10 bg-neutral-100" />
                    <div className="flex-1 text-center"><p className="text-[10px] font-bold text-neutral-400 uppercase tracking-wider mb-1">Date</p><p className="text-sm font-semibold text-neutral-900">{new Date(submission.submitted_at || submission.created_at).toLocaleDateString('en-US', { month: 'short', day: 'numeric', year: 'numeric' })}</p></div>
                    <div className="w-px h-10 bg-neutral-100" />
            ...
            {panel.slug && values.length > 0 && (<button onClick={() => navigate(`/trends/${panel.slug}`)} className="btn btn-outline w-full">View Trends ↗</button>)}
            <NearbyProviders />
            <NearbyProviders type="hospital" title="Nearby Hospitals" />

            {/* ── Health Tools Recommendations ── */}
            {(() => {
                const allTools = [
                ...
                const interpText = (interpretation?.interpretation_text || '').toLowerCase();
                const allText = valNames + ' ' + panelName + ' ' + interpText;
                const ctx = {
                ...
                const tools = matched.length >= 2 ? matched : [...matched, ...allTools.filter(t => !matched.includes(t)).slice(0, 2 - matched.length)];
                return (
                    <div className="card p-5">
                        <div className="flex items-center gap-3 mb-4 pb-4 border-b border-neutral-100"><div className="w-9 h-9 rounded-lg bg-orange-50 flex items-center justify-center text-base text-orange-500">◷</div><div><p className="text-base font-bold text-neutral-900">Health Tools For You</p><p className="text-xs text-neutral-400 mt-0.5">Recommended based on your results</p></div></div>
                        <div className="space-y-2">
                            {tools.map((tool) => (
                                <Link key={tool.title} to={tool.to} className="flex items-center gap-3 bg-neutral-50 rounded-xl p-3 hover:bg-neutral-100 transition-colors">
                                    ...
                                    <div className="flex-1 min-w-0"><p className="text-sm font-semibold text-neutral-900">{tool.title}</p><p className="text-xs text-neutral-500 mt-0.5">{tool.desc}</p></div>
                                    <span className="text-neutral-300 text-lg">›</span>
                                </Link>
                            ))}
                        </div>
                        <Link to="/health-tools" className="block text-center mt-4 text-sm font-bold text-teal-700 hover:text-teal-800">Explore all tools →</Link>
                    </div>
                );
            })()}
