...
<body>
        ...
        <div class="wrap" style="display:flex;flex-direction:column;gap:20px">
            <div class="footer-container" style="border-bottom:1px solid var(--line);padding-bottom:20px">
                ...
                <nav style="display:flex;gap:24px;font-size:0.82rem;flex-wrap:wrap;justify-content:center">
                    ...
                    <a href="{{ route('affiliates') }}" style="color:var(--text-muted);text-decoration:underline;text-underline-offset:4px;text-decoration-color:var(--line);transition:color 0.2s" onmouseover="this.style.color='var(--primary)'" onmouseout="this.style.color='var(--text-muted)'">Affiliates</a>
                    <a href="/clinical-validation" style="color:var(--text-muted);text-decoration:underline;text-underline-offset:4px;text-decoration-color:var(--line);transition:color 0.2s" onmouseover="this.style.color='var(--primary)'" onmouseout="this.style.color='var(--text-muted)'">Clinical Validation</a>
    ...
    <a
        class="wa-float"
        href="https://wa.me/2349022239628?text={{ urlencode('Hello HealthIntel, I need help.') }}"
        target="_blank"
    ...
    <style>
        ...
        .wa-float:hover { transform: translateY(-2px) scale(1.05); box-shadow: 0 10px 28px rgba(37, 211, 102, 0.5); }
        ...
        @supports (padding-bottom: env(safe-area-inset-bottom)) {
            .pwa-float { bottom: calc(84px + env(safe-area-inset-bottom)); }
        }
    </style>

    <script>
        // ── PWA: Redirect standalone mode away from marketing pages ──
        (function() {
            const isStandalone = window.matchMedia('(display-mode: standalone)').matches 
                || window.navigator.standalone === true;
            if (isStandalone) {
                // In the installed PWA, frontend pages are replaced by the SPA
                @auth
        ...
        (function() {
            ...
            // Hide immediately if already installed (standalone mode) or on iOS (no native API)
            if (isStandalone) {
            ...
            // Start hidden — only show when native install prompt becomes available
            ...
            window.addEventListener('beforeinstallprompt', (e) => {
                ...
                // Show the button now that native install is available
                btn.style.display = '';
                btnText.textContent = 'Install app';
            });

            // Tap to install — fires the native browser install dialog (the "confirmation")
            btn.addEventListener('click', async () => {
                if (!deferredInstallPrompt) return;
                // Android/Chrome — fire the native install prompt with browser confirmation
